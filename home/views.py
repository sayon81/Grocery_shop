...
def order(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            post_product_price = request.POST.get('product_price', str(price))
            ...
            return redirect(f"/payment/?{query_params}")
    else:
        form = GroceryOrderForm()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                # Redirect to next page if specified, otherwise dashboard with namespace
...
def logout_view(request):
    ...
    return redirect('home:index')  # Fixed: Added namespace
